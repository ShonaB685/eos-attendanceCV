...
warmup_pipeline(pipeline["detector"], pipeline["aligner"], pipeline["embedder"], pipeline["antispoof_sessions"])
...
def api_enroll():
    ...
    for data_url in data_urls:
        ...
        if frame is None:
            skipped += 1
            continue
        faces = detect_faces(pipeline["detector"], frame)
        if len(faces) != 1:
            skipped += 1
            continue
        aligned = pipeline["aligner"].alignCrop(frame, faces[0])
    ...
    if not re_enroll_id:
        sample_embeddings = [embed_face(pipeline["embedder"], face) for face in aligned_faces[:10]]
        match_id, match_sim = find_existing_match(sample_embeddings, pipeline["gallery"])
    ...
    pipeline["gallery"][student_id] = retrain_student(student_id, pipeline["embedder"])
...
def api_delete_student():
    ...
    pipeline["gallery"].pop(student_id, None)
...
def api_mark():
    ...
    total_banked = resolve_banking(pipeline["gallery"], all_candidates, len(data_urls), pipeline["embedder"])
